export type TabelaSalarialRow = {
  grade: number;
  nivel_nome: string;
  piso_80: number | null;
  p90: number | null;
  mediana: number;
  p110: number | null;
  teto_120: number | null;
  tabela_origem: string;
};

export type FaixaSalarial =
  | ({ status: "ok" } & TabelaSalarialRow)
  | { status: "nao_encontrado" };

export function formatarReal(v: number) {
  return v.toLocaleString("pt-BR", { style: "currency", currency: "BRL" });
}

export function buscarFaixaSalarial(
  grade: number,
  tabela: TabelaSalarialRow[],
): FaixaSalarial {
  // Cruzamento direto: a tabela cobre os grades 1–22 e o modelo de pesagem
  // nunca ultrapassa o grade 20.
  const row = tabela.find((r) => r.grade === grade);
  if (!row) return { status: "nao_encontrado" };
  return { status: "ok", ...row };
}

export function gerarTextoFluido(input: {
  cargoNome: string;
  diretoriaArea: string;
  jaOcupado: boolean;
  pontos: number;
  grade: number;
  faixaMin: number;
  faixaMax: number;
  faixaSalarial: FaixaSalarial;
}) {
  const situacao = input.jaOcupado ? "cargo já ocupado" : "cargo novo, sem ocupante";

  const nivelNome =
    input.faixaSalarial.status === "ok" ? input.faixaSalarial.nivel_nome : null;

  let paragrafoSalarial: string;
  if (input.faixaSalarial.status === "ok") {
    const f = input.faixaSalarial;
    paragrafoSalarial =
      `Segundo a Tabela Salarial Chlorum 2026 (Corporativo), este grade prevê ` +
      `salário-base mensal com piso de ${formatarReal(Number(f.piso_80 ?? f.mediana))} ` +
      `(80% da mediana), mediana de ${formatarReal(Number(f.mediana))} e teto de ` +
      `${formatarReal(Number(f.teto_120 ?? f.mediana))} (120% da mediana).`;
  } else {
    paragrafoSalarial =
      "Faixa salarial não localizada na Tabela Salarial Chlorum 2026 para este grade.";
  }

  return (
    `O cargo de ${input.cargoNome} (${input.diretoriaArea}), avaliado como ${situacao}, ` +
    `foi pré-pesado em ${Math.round(input.pontos).toLocaleString("pt-BR")} pontos, ` +
    `posicionando-se no Grade ${input.grade} da estrutura de cargos da Chlorum ` +
    `(faixa de ${input.faixaMin}–${input.faixaMax} pontos)` +
    (nivelNome ? `, correspondente ao nível ${nivelNome}` : "") +
    `. ${paragrafoSalarial} ` +
    `Este é um resultado de pré-pesagem — cargos-fronteira entre grades devem ser ` +
    `validados no processo oficial antes de qualquer comunicação de banda salarial definitiva.`
  );
}
